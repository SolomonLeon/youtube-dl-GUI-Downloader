...
def choiseDownload(url,method="Default Download"):#Split the download command
    if url == "":
        tkinter.messagebox.showwarning(title='Bad parameter', message="You have to type a url.")
    elif method=="Default Download":
        commandWindow(command=getdownloadCommand(url=url,method="Default Download"))
        ...
        commandWindow(command=getdownloadCommand(url=url,method="Custom",parameter="-x --audio-format mp3"))
...
def getdownloadCommand(url,method="Default Download",parameter=None,sub=True):#return command
    #accept: "Default Download","Return Infor","Custom"
    #I didn't add Multi-system support into this Beta version. Maybe I will add it into next version. :-P
...
def windowsDownload(parameter,proxy,savedir,url,sub):
    if parameter == None:
        parameter = ""
    command = ".\\exefiles\\youtube-dl "+parameter+" "+proxy+" "+savedir+" "+sub+" "+url
